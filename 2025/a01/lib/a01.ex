defmodule A01 do
  ...
  def solve(input) do
    ...
    |> String.split("\n")
    |> Enum.map(fn move ->
      {String.at(move, 0), String.slice(move, 1..-1//1) |> String.to_integer()}
    end)
    |> part1()
    |> part2()
  ...
  def part1(moves) do
    ...
    answer =
      Enum.reduce(moves, [50], fn move, position_list ->
        [move_to(move, hd(position_list)) | position_list]
      end)
      |> Enum.filter(&(&1 == 0))
      |> length()
      |> dbg
  ...
  def part2(moves) do
    answer_input =
      Enum.reduce(moves, [{50, 0}], fn move, position_list ->
        [move_to2(move, hd(position_list)) | position_list]
      end)
  ...
  def move_to({"L", distance}, position) do
    (position - distance + 100) |> rem(100)
  end

  def move_to({"R", distance}, position) do
